...
pip install tensorflow scikit-learn textblob newspaper3k vectorbt py_vollib feedparser
==================================================
...
# ============================================================================
# ML ENHANCEMENT MODULE IMPORT
# ============================================================================
try:
    from pm_ml_enhanced import (
        MLEnhancer, 
    ...
    ML_AVAILABLE = False
    ml_enhancer = None
    print("⚠️ ML module not found. Advanced features disabled.")
...
def detect_emergency_exit(result, market_health):
    ...
    vix = market_health['vix'] if market_health else 15
     # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # VIX-ADJUSTED THRESHOLDS FOR SWING TRADING
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    if vix < 18:
        # Normal volatility - standard thresholds
        sl_risk_emergency = 60
        sl_risk_high = 50
        pnl_emergency = -2
    elif vix <= 22:
        # Slightly elevated - REDUCE SENSITIVITY for swing trades
        sl_risk_emergency = 70  # Was 60, now 70 (less sensitive)
        sl_risk_high = 60       # Was 50, now 60
        pnl_emergency = -3      # Was -2%, now -3% (more tolerance)
    elif vix <= 25:
        # High volatility - slightly tighter
        sl_risk_emergency = 55
        sl_risk_high = 45
        pnl_emergency = -2
    else:
        # Crisis mode (VIX > 25) - aggressive exits
        sl_risk_emergency = 50
        sl_risk_high = 40
        pnl_emergency = -1.5
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if ML_AVAILABLE:
            capabilities = ml_enhancer.get_capabilities()
            
            enable_ml = st.checkbox("Enable ML Features", value=False, 
                                   help="Use AI/ML for predictions and analysis")
            
            if enable_ml:
                enable_lstm = st.checkbox("📈 LSTM Price Prediction", 
                                         value=capabilities['lstm'],
                                         disabled=not capabilities['lstm'])
                enable_sentiment = st.checkbox("📰 News Sentiment", 
                                              value=capabilities['sentiment'],
                                              disabled=not capabilities['sentiment'])
                enable_anomaly = st.checkbox("🔍 Anomaly Detection", 
                                            value=capabilities['anomaly'],
                                            disabled=not capabilities['anomaly'])
                enable_ml_strategy = st.checkbox("🎯 AI Strategy Recommendation", 
                                                value=True)
                
                if not capabilities['lstm']:
                    st.caption("⚠️ Install: pip install tensorflow")
                if not capabilities['sentiment']:
                    st.caption("⚠️ Install: pip install textblob")
            else:
                enable_lstm = False
            ...
            st.warning("⚠️ ML module not installed")
            st.code("pip install tensorflow scikit-learn textblob")
            enable_ml = False
            enable_lstm = False
            enable_sentiment = False
            enable_anomaly = False
            enable_ml_strategy = False
        enable_correlation = st.checkbox("Correlation Analysis", value=True,
                                        help="Analyzes how your stocks move together. Helps identify concentration risk")
        ...
        return {
            ...
            'enable_ml': enable_ml if ML_AVAILABLE else False,
            'enable_lstm': enable_lstm if ML_AVAILABLE else False,
            'enable_sentiment': enable_sentiment if ML_AVAILABLE else False,
            'enable_anomaly': enable_anomaly if ML_AVAILABLE else False,
            'enable_ml_strategy': enable_ml_strategy if ML_AVAILABLE else False
        }
...
def main():
    ...
    for i, (_, row) in enumerate(portfolio.iterrows()):
        ...
        if result:
            # ✅ ADD ML ENHANCEMENTS
            if settings.get('enable_ml', False) and ML_AVAILABLE:
                ml_enhancements = ml_enhancer.enhance_analysis(
                    result,
                    enable_lstm=settings.get('enable_lstm', False),
                    enable_sentiment=settings.get('enable_sentiment', False),
                    enable_anomaly=settings.get('enable_anomaly', False)
                )
                result['ml'] = ml_enhancements
            else:
                result['ml'] = None
    ...
    with tab1:
        ...
        for r in sorted_results:
            ...
            with st.expander(
                ...
                # ============================================================
                # ML ENHANCEMENTS DISPLAY (NEW!)
                # ============================================================
                if r.get('ml') and r['ml'].get('ml_available'):
                    st.divider()
                    st.markdown("### 🤖 AI/ML Analysis")
                    
                    ml_cols = st.columns([1, 1, 1])
                    
                    # LSTM Prediction
                    with ml_cols[0]:
                        if r['ml'].get('lstm_prediction'):
                            html = format_ml_prediction(r['ml']['lstm_prediction'])
                            if html:
                                st.markdown(html, unsafe_allow_html=True)
                    
                    # Sentiment
                    with ml_cols[1]:
                        if r['ml'].get('sentiment'):
                            html = format_sentiment(r['ml']['sentiment'])
                            if html:
                                st.markdown(html, unsafe_allow_html=True)
                                
                                # Show headlines
                                if r['ml']['sentiment'].get('headlines'):
                                    with st.expander("📰 Recent Headlines", expanded=False):
                                        for h in r['ml']['sentiment']['headlines']:
                                            sentiment_emoji = {
                                                'POSITIVE': '👍',
                                                'NEGATIVE': '👎',
                                                'NEUTRAL': '➖'
                                            }.get(h['sentiment'], '•')
                                            st.caption(f"{sentiment_emoji} {h['text']}")
                    
                    # Anomaly Detection
                    with ml_cols[2]:
                        if r['ml'].get('anomaly'):
                            anomaly = r['ml']['anomaly']
                            if anomaly.get('has_anomaly'):
                                ...
                                color = risk_colors.get(anomaly['risk_level'], '#6c757d')
                                
                                st.markdown(f"""
                                <div style='background:{color}20; padding:15px; border-radius:10px; 
                                            border-left:4px solid {color};'>
                                    <h4 style='margin:0;'>🔍 Anomaly Detected</h4>
                                    <p style='margin:5px 0;'><strong>Type:</strong> {anomaly['anomaly_type']}</p>
                                    <p style='margin:5px 0;'><strong>Risk:</strong> {anomaly['risk_level']}</p>
                                    <p style='margin:5px 0;'>{anomaly['description']}</p>
                                </div>
                                """, unsafe_allow_html=True)
                    
                    # AI Strategy Recommendation
                    if r['ml'].get('strategy_recommendation'):
                        st.divider()
                        html = format_strategy(r['ml']['strategy_recommendation'])
                        if html:
                            st.markdown(html, unsafe_allow_html=True)
    ...
    with tab5:
        ...
        if not settings.get('enable_ml', False):
            st.info("💡 Enable ML features in the sidebar to see AI-powered insights")
        elif not ML_AVAILABLE:
            st.warning("⚠️ ML module not installed. Run: pip install tensorflow textblob")
        else:
            ml_results = [r for r in results if r.get('ml') and r['ml'].get('ml_available')]
            
            if not ml_results:
                st.info("📊 ML analysis in progress... Refresh to see results")
            else:
                for r in ml_results:
                    with st.expander(f"🤖 {r['ticker']} - AI Analysis", expanded=True):
